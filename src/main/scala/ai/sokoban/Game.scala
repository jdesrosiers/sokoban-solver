...
case class Game(walls: Set[Point], storage: Set[Point], restricted: Set[Point], reachable: Set[Point]) {
    ...
    def isBoxFrozen(game: Game, state: GameState, box: Point): Boolean = {
      def isFrozen(a: Point, b: Point) =
        (game.isWall(a) || game.isWall(b)) ||
          ...
          (state.isBox(a) && isBoxFrozen(Game(game.walls + box, storage, restricted, reachable), state, a)) ||
          (state.isBox(b) && isBoxFrozen(Game(game.walls + box, storage, restricted, reachable), state, b))
