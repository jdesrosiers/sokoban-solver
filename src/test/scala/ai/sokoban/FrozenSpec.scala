...
class FrozenSpec extends FlatSpec with Matchers {
  "Frozen U-L" should "not be allowed" in {
    val game = Game(Point(2, 4), Set(Point(1, 1), Point(2, 1)), Set())
  ...
  "Frozen D-L" should "not be allowed" in {
    val game = Game(Point(2, 4), Set(Point(1, 4), Point(2, 4)), Set())
  ...
  "Frozen L-U" should "not be allowed" in {
    val game = Game(Point(4, 2), Set(Point(1, 1), Point(1, 2)), Set())
  ...
  "Frozen R-U" should "not be allowed" in {
    val game = Game(Point(4, 2), Set(Point(4, 1), Point(4, 2)), Set())
