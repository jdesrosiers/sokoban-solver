...
class BreadthFirstGraphFrontierSpec extends FlatSpec with BeforeAndAfterEach with Matchers {

	private var frontier: Frontier[String] = _

	override def beforeEach() {
		frontier = new BreadthFirstGraphFrontier[String]()
	}
  ...
  "A frontier" should "emit nodes in FIFO" in {
    val node1 = Node(null, 'op, "one", 0, 0)
